...
describe('chunking', () => {
    ...
    it('every chunk has a non-empty sectionTitle and a positive pageRef', () => {
        for (const chunk of chunks) {
            expect(chunk.sectionTitle.trim().length, `Chunk ${chunk.id} has empty sectionTitle`).toBeGreaterThan(0);
            expect(chunk.pageRef, `Chunk ${chunk.id} has non-positive pageRef`).toBeGreaterThan(0);
        }
    });
