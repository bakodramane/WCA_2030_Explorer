...
function isHighPriority(pageNum: number, totalPages: number): boolean {
  const glossaryStart = Math.max(1, totalPages - 30);
  return (
    STATIC_HIGH_RANGES.some(([lo, hi]) => pageNum >= lo && pageNum <= hi) ||
...
async function main(): Promise<void> {
  ...
  type Section = { title: string; pageRef: number; body: string[] };

  const sections: Section[] = [{ title: 'Front Matter', pageRef: 1, body: [] }];
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    const priority: 'high' | 'normal' = isHighPriority(sec.pageRef, totalPages)
      ? 'high'
      : 'normal';
